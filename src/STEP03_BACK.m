...
function [STEM_data] = STEP03_BACK(STEM_data) 
    ...
    for a2 = STEM_data.numPlanes:-1:1
        ...
        %STEM_data.grad(:,:,STEM_data.z_bin*(a2-1)+1) = +1i*STEM_data.sigma*conj_trans(:,:,a2)...
            %    .*conj(STEM_data.save_wave_f(:,:,a2)).*back_wave_f;
        STEM_data.grad(:,:,STEM_data.z_bin*(a2-1)+1) = +1i*conj_trans(:,:,a2)...
                .*conj(STEM_data.save_wave_f(:,:,a2)).*back_wave_f;

        if a2 < STEM_data.numPlanes
            STEM_data.grad(:,:,STEM_data.z_bin*(a2-1)+1:STEM_data.z_bin*a2) = repmat(STEM_data.grad(:,:,STEM_data.z_bin*(a2-1)+1), [1, 1, STEM_data.z_bin]);
        else
            STEM_data.grad(:,:,STEM_data.z_bin*(a2-1)+1:size(STEM_data.rec,3)) = repmat(STEM_data.grad(:,:,STEM_data.z_bin*(a2-1)+1), [1, 1, size(STEM_data.rec,3)-STEM_data.z_bin*(a2-1)]);
        end
