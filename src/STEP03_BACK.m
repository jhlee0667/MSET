...
function [STEM_data] = STEP03_BACK(STEM_data) 
    ...
    %STEM_data.tmp_probe_wfn = STEM_data.tmp_probe_wfn - STEM_data.step_size(2) .* fftshift((ifftn(ifftshift(back_wave_f))));
    ...
    %STEM_data.row = STEM_data.row + STEM_data.step_size(3) * real(sum(de_probex.* fftshift((ifftn(ifftshift(back_wave_f)))),[1 2]));
    %STEM_data.col = STEM_data.col + STEM_data.step_size(3) * real(sum(de_probey.* fftshift((ifftn(ifftshift(back_wave_f)))),[1 2]));
