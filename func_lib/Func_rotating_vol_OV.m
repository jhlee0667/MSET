...
% using splinterp3 (https://github.com/apryor6/splinterp)

function RotVol = Func_rotating_vol_OV(Vol, vec1, vec2, vec3 ,phi, theta, psi, OV, use_cpp)
    if nargin <= 8
        use_cpp = 0;
    end
    ...
    % binning
    if OV>1
        %RotVol = Func_real_intepolation_OV_vector(RotVol,[OV,OV,1]);
        OV_vol = Func_real_intepolation_OV(Vol,OV);
    else
        OV_vol = Vol;
    end
...
%    RotVol(isnan(RotVol)) = 0;
    ...
    RotVol = imresize3(RotVol,[size(RotVol,1), size(RotVol,2), size(RotVol,3)*1/OV], 'box');
