% calculate_S_matrices_for3DFourierShearRot_perm_backward
...
% calculate_S_matrices_for3DFourierShearRot_perm_backward calculates the backward
% transform shear matrices and coefficients for 3D rotation
% Reference: Welling et al., Graphical Models 68 (2006) 356-370

function [S_1, S_2, S_3, S_4, Coeffs, absSum] = calculate_S_matrices_for3DFourierShearRot_perm_backward_ver1(X_inp,Y_inp,Z_inp,W,numPerm)
    ...
    if Singular
        ...
        if mod(numPerm,3) == 0
            S_1 = S_y_MAT(-g,-h,delY);
            S_2 = S_x_MAT(-e,-f,delX);
            S_3 = S_z_MAT(-c,-d,delZ);
            S_4 = S_y_MAT(-a,-b,delY);
        elseif mod(numPerm,3) == 1
            S_1 = S_z_MAT(-g,-h,delZ);
            S_2 = S_y_MAT(-e,-f,delY);
            S_3 = S_x_MAT(-c,-d,delX);
            S_4 = S_z_MAT(-a,-b,delZ);
        elseif mod(numPerm,3) == 2
            S_1 = S_x_MAT(-g,-h,delX);
            S_2 = S_z_MAT(-e,-f,delZ);
            S_3 = S_y_MAT(-c,-d,delY);
            S_4 = S_x_MAT(-a,-b,delX);
        end    
